...
public class ResizeDialogController implements IResizeListener {
	...
	/*
			...
			this.worldException = null;
			this.modelThread.enqueueAndWait(new Runnable() {
				@Override
				public void run() {
					try {
						switch(event.getDirection()) {
							case WorldResizeEvent.RESIZE_NORTH:
								if(event.isExtend()) {
								...
								break;
							case WorldResizeEvent.RESIZE_SOUTH:
								if(event.isExtend()) {
								...
								break;
							case WorldResizeEvent.RESIZE_WEST:
								if(event.isExtend()) {
								...
								break;
							case WorldResizeEvent.RESIZE_EAST:
								if(event.isExtend()) {
						...
						sizeX = world.getX();
						sizeY = world.getY();
						data = world.exportSegmentUsage();
					} catch(WorldException exception) {
						worldException = exception;
					}
				}
			});
			if(null != this.worldException) {
				JOptionPane.showMessageDialog(this.dialog, this.worldException.getMessage(), "World resizing problem", JOptionPane.ERROR_MESSAGE);
			} else {
				this.eventBus.post(new WorldSizeChangedEvent(this.sizeX, this.sizeY));
				this.dialog.setWorldSize(this.sizeX, this.sizeY);
				this.dialog.setMinimapData(this.data);
