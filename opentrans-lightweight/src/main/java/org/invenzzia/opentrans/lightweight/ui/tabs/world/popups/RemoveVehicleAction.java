...
import org.invenzzia.opentrans.visitons.editing.network.RemoveVehicleFromTrackCmd;

@PopupAction(text = "Remove vehicle")
public class RemoveVehicleAction extends AbstractVehicleAction {
	@Override
	protected void doExecute(IEditModeAPI api, VehicleRecord record) {
		try {
			this.history.execute(new RemoveVehicleFromTrackCmd(record));
